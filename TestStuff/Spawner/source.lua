...
local CONST = {
	...
	DEFAULT = {
		CONFIG = {
            ...
            Jumpscare = {
                Enabled = false,
                Image1 = "rbxassetid://11395249153",
                Image2 = "rbxassetid://11395249153",
                Shake = true,
                Sound1 = { SoundId = 1427189017, Volume = 3 },
                Sound2 = { SoundId = 5263560566, Volume = 3 },
                Flashing = {
                    Enabled = true,
                    Color1 = Color3.fromRGB(3, 25, 99),
...
-- Универсальный парсер ссылок (Senior-touch)
local function ParseAsset(asset)
    if type(asset) == "string" then
        if asset:match("^http") and getgenv().LoadCustomAsset then
            local success, result = pcall(function() return getgenv().LoadCustomAsset(asset) end)
            if success and result then return result end
        elseif asset:match("^%d+$") then
...
local function PlayJumpscare(config: any)
    ...
    Background.BackgroundColor3 = config.Flashing.Color1 or Color3.fromRGB(0, 0, 0)
    ...
    Face.Image = ParseAsset(config.Image1)

    Background.Parent = JumpscareGui
    Face.Parent = Background

    local sounds = {}
    local function loadSound(sndConfig)
        if sndConfig and sndConfig.SoundId then
            local s = Instance.new("Sound")
            s.SoundId = ParseAsset(sndConfig.SoundId)
            s.Parent = JumpscareGui
            -- Применяем остальные проперти (Volume, Pitch и т.д.)
            for k, v in next, sndConfig do
                if k ~= "SoundId" then
    ...
    -- Логика тизера (Tease)
    if config.Tease.Enabled then
        local teaseWait = math.random(config.Tease.Min * 10, config.Tease.Max * 10) / 10
        Face.Image = ParseAsset(config.Image2) or Face.Image
        task.wait(teaseWait)
    end
    ...
    -- Эффект мигания фона
    local flashingTask
    if config.Flashing.Enabled then
        flashingTask = task.spawn(function()
            while JumpscareGui.Parent do
                Background.BackgroundColor3 = config.Flashing.Color1
                task.wait(math.random(25, 100) / 1000)
                Background.BackgroundColor3 = config.Flashing.Color2
                task.wait(math.random(25, 100) / 1000)
            end
        end)
    end

    -- Тряска экрана через встроенный шейкер
    if config.Shake then
        Modules.Main_Game.camShaker:ShakeOnce(10, 15, 0.2, 1.5)
    end

    for _, s in next, sounds do
        s:Play()
    end

    -- Анимация скримера (использую scale, чтобы гарантировать перекрытие на любых мониторах)
    TweenService:Create(Face, TweenInfo.new(0.6, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {
        Size = UDim2.fromScale(3, 3), 
        ImageTransparency = 0
    }):Play()

    task.wait(0.7)
    if flashingTask then task.cancel(flashingTask) end
...
local function DamagePlayer(entity: any)
    ...
    if newHealth == 0 then
        -- Вызов скримера, если включен в конфиге
        if config.Jumpscare and config.Jumpscare.Enabled then
            task.spawn(PlayJumpscare, config.Jumpscare)
        end
